  ...
  $self->on(
    message => sub {
      ...
      my $author =
        ...
        ->single({name => $self->session->{username}});
      $data->{author_id} = $author->id;
      $schema->resultset('Page')
        ->update_or_create($data, {key => 'pages_name'},);
      $self->expire('main');
...
sub edit_menu {
  ...
  my %active =
    map { $_ => 1 }
    @{$json->decode($schema->resultset('Menu')->single({name => $name})->list)
    };
  ...
  for my $page (@pages) {
    ...
    exists $active{$id} ? $active : $inactive .=
      sprintf
      qq{<li id="pages-%s"><span class="label label-info">%s</span></li>\n},
      $id, $page->title;
  }
...
sub store_menu {
  ...
  $self->on(
    message => sub {
      ...
      $schema->resultset('Menu')
        ->update({list => $json->encode(\@pages),}, {key => $name});
